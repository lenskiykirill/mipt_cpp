...
size_t get_nested (segment* s, size_t N) {
...
size_t dumb_check (segment* array, size_t N) {
      size_t res = 0;

      for (size_t i=0; i < N; i++)
            for (size_t j=0; j < N; j++)
                  if (i != j) {
                        if ((array[i].left <= array[j].left) && (array[i].right > array[j].right))
                              ++res;
                        else if ((array[i].left < array[j].left) && (array[i].right >= array[j].right))
                              ++res;
                  }
      return res;
}
...
int main () {
      ...
      dst::quick_sort<struct segment> (array, N);

      std::cout << (get_nested (array, N) == (dumb_check (array, N))?"OK":"FAIL") << '\n';
